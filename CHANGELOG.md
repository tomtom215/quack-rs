...
the book and the README and check the book's links, and corrects a claim in
the 0.17.0 notes about aggregates and NULL rows (see Fixed).
...
- **Pitfall L12** (`LESSONS.md`, `book/src/reference/pitfalls.md`): an
  ...
  See the correction under Fixed.
...
- **Breaking:** `QueryResult::next_chunk` returns
  ...
  stays `Ok(None)`.
- **Breaking:** `Value::time`, `time_tz`, `time_ns`, `timestamp`,
  `timestamp_tz`, `timestamp_s`, `timestamp_ms` and `timestamp_ns` return
  `Result<Value, ExtensionError>` and refuse a payload outside the range
  DuckDB's own SQL produces. DuckDB stores any 64-bit payload unchecked, and
  rendering an out-of-range one crashed or aborted the process
  (`Value::time_ns(i64::MAX)`, `Value::timestamp(i64::MIN)`) or printed garbage
  (`Value::time(-1)`). `Value::date` and `Value::interval` stay infallible:
...
- **Breaking:** `OwnedVector::new` returns an error for a capacity above
  `vector::ops::MAX_CAPACITY`, counting the child vectors a nested type
  allocates (see Security).
...
- **Breaking:** `data_chunk_from_arrow` returns `InvalidInput` for a negative
  length or offset, a null `children` pointer, a null child, and a child
  shorter than its parent's offset + length (see Security). Its Safety section
  now also requires that the array conform to the schema: nothing in an Arrow
  array records its type.
...
- **Correction to the 0.17.0 release notes: an aggregate's `update` does
  receive NULL rows.** The 0.17.0 entry for Pitfall L8 ends "Aggregates are
  unaffected — their executor really does filter NULL rows." That is false, and
  so was the matching documentation (`NullHandling`, the `null_handling` setter
  on both aggregate builders, the null-handling book page, Pitfall L8 and the
  first-extension tutorial). DuckDB's `CAPIAggregateUpdate` passes every row of
...
- `Value::list_value` / `array_value` refused a `LIST` or `ARRAY` element type,
  and with it every list of lists and array of arrays, which DuckDB builds. The
  explanatory error for passing the container type instead of the element type
  now appears only when DuckDB itself refuses the value.
- The typed scalar constructors (`map1`, `map2`, `map1_opt`, `map2_opt`,
  `map1_str`, `map2_str`) called DuckDB while building, so they panicked in a
  unit test without a live DuckDB, for example with `MockRegistrar`. They no
  longer do; `register` still checks the types.
...
- **Documentation that was false against the code or DuckDB, third pass:**
  ...
  - Queries and values: a multi-statement string passed to `query()` /
    ...
    `struct_field_names` of a UNION returns an empty name for the tag, then
    the member names.
  - Arrow: DuckDB converts dictionary- and run-end-encoded children rather than
    rejecting them; TIMETZ comes back as TIME without its offset and BIT as
    BLOB. Not fixed, documented in `data_chunk_from_arrow`'s Safety section: an
    absurd array length still aborts the process, because DuckDB allocates the
    chunk before its `try` block.
...
- **Soundness, third pass: more ways safe code, or a malformed input, could
  ...
  - `OwnedVector::new` let DuckDB compute a vector's buffer size with an
    unchecked multiply, so `OwnedVector::new(HUGEINT, 2^60 + 2)` got a 32-byte
    buffer and returned `Ok`, and writing rows within the reported capacity
    overwrote neighbouring heap allocations. **Breaking:** it now walks the
    type, child vectors included, with checked arithmetic, and refuses a
    capacity above `vector::ops::MAX_CAPACITY` before DuckDB allocates (see
    Changed).
  - A `FileHandle` could outlive its database and read freed memory.
    **Breaking:** `FileHandle<'fs>` (see Changed).
  - `data_chunk_from_arrow` handed DuckDB a struct column's children without
    checking them: a null `children` pointer or a null child segfaulted, and a
    child shorter than its parent was read out of bounds. **Breaking:** these
    are now `InvalidInput` errors (see Changed).
