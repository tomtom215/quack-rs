...
pub unsafe fn contains_any_or_invalid(ty: duckdb_logical_type) -> bool {
    ...
    if id == DUCKDB_TYPE_DUCKDB_TYPE_LIST {
        // SAFETY: `ty` is a live LIST type.
        child_bad(unsafe { duckdb_list_type_child_type(ty) })
    } else if id == DUCKDB_TYPE_DUCKDB_TYPE_ARRAY {
        // SAFETY: `ty` is a live ARRAY type.
        child_bad(unsafe { duckdb_array_type_child_type(ty) })
    } else if id == DUCKDB_TYPE_DUCKDB_TYPE_MAP {
        // SAFETY: `ty` is a live MAP type.
        child_bad(unsafe { duckdb_map_type_key_type(ty) })
            // SAFETY: as above.
            || child_bad(unsafe { duckdb_map_type_value_type(ty) })
        ...
        // SAFETY: `ty` is a live STRUCT type and `i < count`.
        (0..count).any(|i| child_bad(unsafe { duckdb_struct_type_child_type(ty, i) }))
        ...
        // SAFETY: `ty` is a live UNION type and `i < count`.
        (0..count).any(|i| child_bad(unsafe { duckdb_union_type_member_type(ty, i) }))
    } else {
        false
    }
}
...
fn child_bad(raw: duckdb_logical_type) -> bool {
    if raw.is_null() {
        return true;
    }
