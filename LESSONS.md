...
| `duckdb_ext_api_v1` field order changes | Both the C++ struct (in `extension_api.hpp`) and the Rust struct (in `libduckdb-sys` bindgen from `duckdb_extension.h`) are generated from the same DuckDB release, so they remain in sync |
